...
public class SummaryDialog extends JDialog{
    ...
    public static class Stats {
        ...
        public final double avgDeliverySeconds;
        public final long fastestDelivery;
        public final long slowestDelivery;
        public final List<String> agentNames;
        public final List<Integer> agentDeliveries;

        public Stats(int delivered,int agents,int maxOrders,long elapsed,
                     double avgWait,double avgDelivery,long fastest,long slowest,
                     List<String> agentNames, List<Integer> agentDeliveries){
            this.totalDelivered= delivered;
            this.totalAgents = agents;
            this.maxOrders = maxOrders;
            this.elapsedSeconds =elapsed;
            this.avgWaitSeconds= avgWait;
            this.avgDeliverySeconds= avgDelivery;
            this.fastestDelivery = fastest;
            this.slowestDelivery = slowest;
    ...
    private JPanel timingStats(Stats s) {
        JPanel card = card();
        JLabel title = sectionLbl("PERFORMANCE BREAKDOWN");
        card.add(title);
        card.add(Box.createVerticalStrut(12));
        double rate = s.elapsedSeconds>0 ? (double) s.totalDelivered /(s.elapsedSeconds/60.0):0;
        card.add(statRow("Avg. wait time",fmt(s.avgWaitSeconds)+"s",AMBER));
        card.add(Box.createVerticalStrut(6));
        card.add(statRow("Avg. delivery time",fmt(s.avgDeliverySeconds) + "s",BLUE));
        card.add(Box.createVerticalStrut(6));
        card.add(statRow("Fastest delivery",s.fastestDelivery >=0 ? s.fastestDelivery+"s":"—", GREEN));
        card.add(Box.createVerticalStrut(6));
        card.add(statRow("Slowest delivery",s.slowestDelivery>=0 ? s.slowestDelivery + "s" : "—",RED));
        card.add(Box.createVerticalStrut(6));
        card.add(statRow("Throughput",String.format("%.1f", rate) +" / min",PURPLE));
        return card;
    }

    private JPanel agentBreakdown(Stats s) {
        JPanel card = card();
        JLabel title = sectionLbl("AGENT LEADERBOARD");
        card.add(title);
        card.add(Box.createVerticalStrut(12));
        ...
        Color[] colors = {
                ...
                new Color(132,204, 22),new Color(56, 189, 248),new Color(251, 113, 133),new Color(52,211, 153)
        };
        ...
        for(int i=0;i<s.agentNames.size();i++){
            ...
            card.add(agentBar(name, count, best, col));
            if(i < s.agentNames.size() - 1) card.add(Box.createVerticalStrut(5));
        }
        return card;
    }
    private JPanel agentBar(String name, int count, int best,Color col){
        JPanel row =new JPanel(new BorderLayout(8,0));
        row.setOpaque(false);
        row.setMaximumSize(new Dimension(440,24));
        ...
        int fillW =best>0 ? (int)((double) count/best *380) :0;
        ...
        JLabel countL =new JLabel(count +" deliveries", SwingConstants.RIGHT);
        countL.setForeground(MUTED);
        countL.setFont(new Font("Consolas", Font.PLAIN, 11));
        countL.setPreferredSize(new Dimension(90,20));

        row.add(nameL, BorderLayout.WEST);
        row.add(barWrap, BorderLayout.CENTER);
        row.add(countL, BorderLayout.EAST);
        return row;
    }
