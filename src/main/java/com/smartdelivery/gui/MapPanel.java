...
public class MapPanel extends JPanel {
    ...
    private static class DeliveryPin {
        volatile GeoPosition position;
        volatile Color color;
        DeliveryPin(GeoPosition pos, Color col) { position = pos; color = col; }
    }
    ...
    private static class AgentMarker {
        ...
        final String status;
        final Color color;
        AgentMarker(GeoPosition p, String s, Color c) { position = p; status = s; color = c; }
    }
    ...
    public MapPanel() {
        ...
        mapViewer.addMouseMotionListener(new MouseMotionAdapter() {
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                GeoPosition newCenter = mapViewer.getTileFactory().pixelToGeo(
                        new Point2D.Double(cx, cy), mapViewer.getZoom());
        ...
        mapViewer.addMouseWheelListener(e -> {
            ...
            mapViewer.setZoom(e.getWheelRotation() < 0 ? Math.max(0, z - 1) : Math.min(10, z + 1));
        });
    ...
    public void updateAgentPosition(String id, Location loc, String status, Color color) {
    ...
    public void updateAgentRoute(String id, List<GeoPosition> route) {
        if (route != null && !route.isEmpty())
            agentRoutes.put(id, new ArrayList<>(route));
        else
            agentRoutes.remove(id);
        SwingUtilities.invokeLater(mapViewer::repaint);
    }

    public void addDeliveryPin(String orderId, Location loc) {
    ...
    public void updateDeliveryPinColor(String orderId, Color color) {
    ...
    public void removeDeliveryPin(String orderId) {
    ...
    private class RoutePainter implements Painter<JXMapViewer> {
        ...
        public void paint(Graphics2D g, JXMapViewer map, int w, int h) {
            ...
            Map<String, List<GeoPosition>> routeSnapshot = new HashMap<>(agentRoutes);
            for (Map.Entry<String, List<GeoPosition>> e : routeSnapshot.entrySet()) {
                AgentMarker marker = agentMarkers.get(e.getKey());
                Color c = marker != null ? marker.color : Color.GRAY;
                List<GeoPosition> route = e.getValue();
    ...
    private class PinPainter implements Painter<JXMapViewer> {
        ...
        public void paint(Graphics2D g, JXMapViewer map, int w, int h) {
            ...
            for (Map.Entry<String, DeliveryPin> e : new HashMap<>(deliveryPins).entrySet()) {
    ...
    private class AgentPainter implements Painter<JXMapViewer> {
        ...
        public void paint(Graphics2D g, JXMapViewer map, int w, int h) {
            ...
            for (Map.Entry<String, AgentMarker> e : new HashMap<>(agentMarkers).entrySet()) {
